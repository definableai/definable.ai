...
description: Turn repeated deterministic tool calls into cache hits.
icon: database
---

Caching is configured at authoring time on the decorator. Once enabled, the runtime stores JSON results keyed by the tool name and arguments.
...
- Use caching for deterministic lookups and transformations.
- Avoid it for side-effecting or freshness-sensitive tools.
