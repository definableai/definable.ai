---
title: Tool Dependencies
sidebarTitle: Dependencies
description: Inject runtime state and services into a tool without exposing them to the model.
icon: puzzle-piece
---

Definable strips special runtime parameters from the tool schema and injects them at execution time. The important names are:

| Parameter | Meaning |
| --- | --- |
| `dependencies` | Services from `AgentConfig(dependencies=...)` |
| `session_state` | Mutable per-run state |
| `run_context` | Run identifiers and retrieved layer context |
| `agent` | The active agent instance |
| `team` | The active team when the tool runs under a team leader |

These are plain parameter names, not underscore-prefixed names.

## Tested example

```python
from definable.agent.run import RunContext
from definable.tool import tool
from definable.tool.function import FunctionCall


@tool
def create_ticket(
    title: str,
    dependencies=None,
    session_state=None,
    run_context=None,
) -> str:
    """Create a ticket using injected runtime objects."""
    dependencies["tickets"].append((title, run_context.run_id, session_state["user"]))
    return f"created:{title}"


function = create_ticket.model_copy(deep=True)
function._dependencies = {"tickets": []}
function._session_state = {"user": "alice"}
function._run_context = RunContext(
    run_id="run-1",
    session_id="session-1",
    user_id="alice",
    session_state=function._session_state,
)

result = FunctionCall(function=function, arguments={"title": "Fix login"}).execute()

assert "dependencies" not in function.parameters["properties"]
assert result.result == "created:Fix login"
assert function._dependencies["tickets"] == [("Fix login", "run-1", "alice")]
```

## Guidance

- Use `AgentConfig(dependencies=...)` to supply services in real runs.
- Keep injected objects out of the model-visible contract.
- Prefer explicit service objects in `dependencies` over global state.

## Related pages

- [`Parameters`](/tools/parameters)
- [`Toolkits`](/toolkits/overview)
