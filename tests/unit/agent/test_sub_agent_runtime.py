...
class TestSpawnAgentInjection:
  def test_spawn_agent_injected_when_sub_agents_true(self):
    ...
    agent = Agent(model=MockModel(responses=["hi"]), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
  ...
  def test_spawn_agent_not_injected_when_sub_agents_none(self):
    ...
    agent = Agent(model=MockModel(responses=["hi"]), config=NO_TRACE)  # type: ignore[arg-type]
    tools = agent._prepare_tools_for_run(_make_ctx())
  ...
  def test_spawn_agent_injected_with_policy(self):
    ...
    agent = Agent(model=MockModel(responses=["hi"]), sub_agents=policy, config=NO_TRACE)  # type: ignore[arg-type]
    tools = agent._prepare_tools_for_run(_make_ctx())
    assert "spawn_agent" in tools
...
class TestSpawnAgentFunctionObject:
  ...
  def test_function_has_tcb_list(self):
    ...
    assert isinstance(fn._tcb_list, list)  # type: ignore[attr-defined]
    assert len(fn._tcb_list) == 0  # type: ignore[attr-defined]
...
class TestSpawnAgentEntrypoint:
  ...
  async def test_spawn_returns_child_content(self):
    ...
    parent = Agent(model=MockModel(responses=["child answer"]), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
  ...
  async def test_spawn_with_instructions(self):
    ...
    parent = Agent(model=MockModel(responses=["expert answer"]), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
    fn = _build_spawn_agent_function(parent, SubAgentPolicy())
    assert fn.entrypoint is not None
  ...
  async def test_spawn_with_context(self):
    ...
    parent = Agent(model=MockModel(responses=["contextual answer"]), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
    fn = _build_spawn_agent_function(parent, SubAgentPolicy())
    assert fn.entrypoint is not None
  ...
  async def test_spawn_handles_failure_gracefully(self):
    ...
    parent = Agent(model=MockModel(side_effect=failing_side_effect), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
    fn = _build_spawn_agent_function(parent, SubAgentPolicy())
    assert fn.entrypoint is not None
  ...
  async def test_spawn_returns_empty_on_none_content(self):
    ...
    parent = Agent(model=MockModel(side_effect=none_content), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
    fn = _build_spawn_agent_function(parent, SubAgentPolicy())
    assert fn.entrypoint is not None
...
class TestTCBTracking:
  ...
  async def test_tcb_created_on_spawn(self):
    parent = Agent(model=MockModel(responses=["done"]), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
    fn = _build_spawn_agent_function(parent, SubAgentPolicy())
    assert fn.entrypoint is not None
    await fn.entrypoint(goal="test")
    assert len(fn._tcb_list) == 1  # type: ignore[attr-defined]

  @pytest.mark.asyncio
  async def test_tcb_completed_state(self):
    parent = Agent(model=MockModel(responses=["result"]), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
    fn = _build_spawn_agent_function(parent, SubAgentPolicy())
    assert fn.entrypoint is not None
    await fn.entrypoint(goal="test")
    tcb = fn._tcb_list[0]  # type: ignore[attr-defined]
  ...
  async def test_tcb_failed_state(self):
    ...
    parent = Agent(model=MockModel(side_effect=failing), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
    fn = _build_spawn_agent_function(parent, SubAgentPolicy())
    assert fn.entrypoint is not None
    await fn.entrypoint(goal="test")
    tcb = fn._tcb_list[0]  # type: ignore[attr-defined]
  ...
  async def test_tcb_goal_recorded(self):
    parent = Agent(model=MockModel(responses=["ok"]), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
    fn = _build_spawn_agent_function(parent, SubAgentPolicy())
    assert fn.entrypoint is not None
  ...
  async def test_tcb_has_unique_id(self):
    ...
    assert len(fn._tcb_list) == 2  # type: ignore[attr-defined]
    assert fn._tcb_list[0].id != fn._tcb_list[1].id  # type: ignore[attr-defined]
...
class TestSubAgentEventEmission:
  ...
  async def test_completed_event_emitted(self):
    ...
    parent = Agent(model=MockModel(responses=["answer"]), sub_agents=True, config=NO_TRACE)  # type: ignore[arg-type]
...
class TestConcurrency:
  ...
  async def test_semaphore_limits_concurrent(self):
    ...
    Agent.arun = patched_arun  # type: ignore[method-assign]
    try:
      assert fn.entrypoint is not None
      ...
      Agent.arun = original_arun  # type: ignore[method-assign]
  ...
  async def test_parallel_spawn_agents(self):
    ...
    Agent.arun = slow_arun  # type: ignore[method-assign]
    try:
      assert fn.entrypoint is not None
...
class TestInheritance:
  ...
  async def test_inherit_tools_true(self):
    ...
    parent = Agent(
      model=MockModel(side_effect=capture_tools),  # type: ignore[arg-type]
      tools=[sample_tool],
      sub_agents=True,
      config=NO_TRACE,
    )
...
class TestEndToEnd:
  ...
  async def test_agent_arun_with_spawn(self):
    ...
    parent = Agent(
      model=MockModel(side_effect=parent_side_effect),  # type: ignore[arg-type]
      sub_agents=True,
      config=NO_TRACE,
    )
