...
def openai_model():
  ...
  if not api_key:
    pytest.fail("OPENAI_API_KEY not set — required for integration tests. Run: source .env.test")
...
def deepseek_model():
  ...
  if not api_key:
    pytest.fail("DEEPSEEK_API_KEY not set — required for integration tests. Run: source .env.test")
...
def moonshot_model():
  ...
  if not api_key:
    pytest.fail("MOONSHOT_API_KEY not set — required for integration tests. Run: source .env.test")
...
def xai_model():
  ...
  if not api_key:
    pytest.fail("XAI_API_KEY not set — required for integration tests. Run: source .env.test")
...
def voyage_embedder():
  ...
  if not api_key:
    pytest.fail("VOYAGEAI_API_KEY not set — required for integration tests. Run: source .env.test")
