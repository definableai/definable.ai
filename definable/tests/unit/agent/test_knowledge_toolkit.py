...
class TestKnowledgeToolkitInit:
  ...
  def test_is_toolkit_subclass(self):
    kb = FakeKnowledge()
    tk = KnowledgeToolkit(knowledge=kb)
    assert isinstance(tk, Toolkit)

  def test_stores_knowledge_ref(self):
    kb = FakeKnowledge()
    tk = KnowledgeToolkit(knowledge=kb)
    assert tk._knowledge is kb

  def test_default_top_k(self):
    tk = KnowledgeToolkit(knowledge=FakeKnowledge())
    assert tk._top_k == 5

  def test_custom_top_k(self):
    tk = KnowledgeToolkit(knowledge=FakeKnowledge(), top_k=10)
    assert tk._top_k == 10
  ...
  def test_rerank_disabled(self):
    tk = KnowledgeToolkit(knowledge=FakeKnowledge(), rerank=False)
...
class TestSearchKnowledgeTool:
  ...
  def test_returns_results(self):
    ...
    result = search_tool.entrypoint(query="PTO policy")
  ...
  def test_no_results(self):
    ...
    search_tool = next(t for t in tk.tools if t.name == "search_knowledge")
    result = search_tool.entrypoint(query="nothing")
    assert "No relevant documents found" in result

  def test_custom_top_k(self):
    docs = [FakeDoc(content=f"Doc {i}") for i in range(10)]
    kb = FakeKnowledge(docs=docs)
    tk = KnowledgeToolkit(knowledge=kb, top_k=3)

    search_tool = next(t for t in tk.tools if t.name == "search_knowledge")
    result = search_tool.entrypoint(query="test")
  ...
  def test_top_k_override(self):
    ...
    search_tool = next(t for t in tk.tools if t.name == "search_knowledge")
    result = search_tool.entrypoint(query="test", top_k=1)
