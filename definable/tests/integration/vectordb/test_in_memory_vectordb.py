"""
Integration tests for InMemoryVectorDB with a real OpenAI embedder.

Rules:
  - NO MOCKS — tests real insert/search/delete operations
  - InMemoryVectorDB uses a REAL embedder (OpenAI) for search query embedding
...
class TestInMemoryVectorDB:
  ...
  Documents are pre-embedded with real embeddings (API cost for inserts).
  search() also calls the embedder (one API call per search query).
  """
