...
```python Async
response = await model.ainvoke(messages=[{"role": "user", "content": "Hello!"}])
print(response.content)
```

```python Streaming
for chunk in model.invoke_stream(messages=[{"role": "user", "content": "Hello!"}]):
    if chunk.content:
        print(chunk.content, end="", flush=True)
```

```python Async Streaming
async for chunk in model.ainvoke_stream(messages=[{"role": "user", "content": "Hello!"}]):
    if chunk.content:
        print(chunk.content, end="", flush=True)
```
</CodeGroup>
