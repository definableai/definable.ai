...
<CardGroup cols={2}>
  <Card title="Models" icon="microchip" href="/models/overview">
    Unified interface for OpenAI, DeepSeek, Moonshot, xAI, Anthropic, Mistral, Google Gemini, Perplexity, Ollama, OpenRouter, and any OpenAI-compatible provider. Supports streaming, structured output, vision, and audio.
  </Card>
  ...
  <Card title="Interfaces" icon="message-bot" href="/interfaces/overview">
    Connect agents to messaging platforms like Telegram and Discord. Automatic session management, hooks, identity resolution, and multi-interface serving.
  </Card>
  <Card title="Memory" icon="brain" href="/memory/overview">
    Session-history memory with automatic recall and auto-summarization. Supports SQLite, file-based, and in-memory backends.
  </Card>
...
flowchart TD
  ...
  subgraph intLayer [Interfaces]
    direction LR
    Telegram --- Discord --- CustomInt["Custom"]
  end
...
- **Multi-provider**: Switch between OpenAI, DeepSeek, Moonshot, xAI, Anthropic, Mistral, Google Gemini, Perplexity, Ollama, OpenRouter, or any OpenAI-compatible API with one line.
...
- **Interfaces**: Deploy agents to Telegram and Discord with sessions, hooks, identity resolution, and multi-interface serving.
- **Memory**: Session-history memory with automatic recall and auto-summarization. 3 storage backends (SQLite, file-based, in-memory).
