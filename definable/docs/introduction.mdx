...
<CardGroup cols={2}>
  ...
  <Card title="Memory" icon="brain" href="/memory/overview">
    LLM-driven persistent memory with automatic recall and cross-session context. Supports SQLite, PostgreSQL, and in-memory backends.
  </Card>
...
- **Memory**: LLM-driven persistent memory with automatic recall and 3 storage backends (SQLite, PostgreSQL, in-memory).
