...
<Note>
These are **Knowledge readers** (`definable.knowledge.reader`) for the RAG document ingestion pipeline. If you need to extract text from files attached to agent messages (PDF, DOCX, XLSX, audio) before LLM processing, see [File Readers](/readers/overview) instead.
</Note>
...
```python
from definable.knowledge.reader import PDFReader
...
```python
from definable.knowledge.reader import URLReader
...
```python
from definable.knowledge.reader import Reader
...
from definable.knowledge.reader import TextReader, PDFReader, URLReader
