...
<Note>
Requires `croniter`: `pip install croniter` or `pip install 'definable-ai[cron]'`
</Note>
...
<Note>
Requires `fastapi` and `uvicorn`: `pip install fastapi uvicorn` or `pip install 'definable-ai[serve]'`
</Note>
