...
````mermaid
flowchart TD
  UserInput["User Input"] --> BuildMsgs["1. Build messages"]
  BuildMsgs --> CallModel["2. Call model with tools"]
  CallModel --> ModelResp["3. Model returns response"]
  ModelResp --> Decision{Text or Tool calls?}
  Decision -->|Text| FinalResp["Final Response"]
  Decision -->|Tool calls| ExecTools["4. Execute each tool call"]
  ExecTools --> AddResults["5. Add results to messages"]
