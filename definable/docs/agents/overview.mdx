...
flowchart TD
  ...
  ReadFiles --> RetrieveKnowledge["Retrieve knowledge"]
  RetrieveKnowledge --> RecallMemory["Recall memory"]
  RecallMemory --> ThinkingPhase{"Thinking enabled?"}
  ThinkingPhase -->|Yes| Think["Thinking phase (analyze + plan)"]
  Think --> BuildMsgs["Build messages"]
