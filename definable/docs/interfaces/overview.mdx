...
An **Interface** connects a Definable agent to an external messaging platform — Telegram, Discord, Desktop, CLI, or any custom channel. It handles the full lifecycle: receiving messages, managing sessions, resolving user identity, running the agent, and sending responses back.
