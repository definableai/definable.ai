...
```python
from definable.agent.interface import SessionManager, TelegramInterface
...
interface = TelegramInterface(agent=agent, bot_token="...", session_ttl_seconds=300)  # 5 minutes

# Long sessions for complex workflows
interface = TelegramInterface(agent=agent, bot_token="...", session_ttl_seconds=86400)  # 24 hours
```
