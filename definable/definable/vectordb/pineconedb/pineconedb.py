...
from definable.utils.log import log_debug, log_warning, logger
...
class PineconeDb(VectorDB):
  ...
  async def async_upsert(
    ...
    # Upsert all vectors
    await asyncio.to_thread(self._upsert_vectors, all_vectors, namespace or self.namespace, batch_size, show_progress)

    log_debug(f"Finished async upsert of {len(documents)} documents")

  async def _prepare_vectors(self, documents: List[Document], content_hash: str, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    """Prepare vectors for upsert. Documents arrive pre-embedded."""
    vectors = []
  ...
  def delete_by_id(self, doc_id: str) -> bool:
    ...
    try:
      ...
      log_warning(f"Error deleting document with ID {doc_id}: {e}")
      return False
  ...
  def id_exists(self, doc_id: str) -> bool:
    ...
    try:
      ...
      log_warning(f"Error checking if ID {doc_id} exists: {e}")
      return False
