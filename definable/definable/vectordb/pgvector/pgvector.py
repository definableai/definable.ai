...
class PgVector(VectorDB):
  ...
  def id_exists(self, doc_id: str) -> bool:
    ...
    """
    return self._record_exists(self.table.c.id, doc_id)
  ...
  def _get_document_record(self, doc: Document, filters: Optional[Dict[str, Any]] = None, content_hash: str = "") -> Dict[str, Any]:
    cleaned_content = self._clean_content(doc.content)
    # Include content_hash in ID to ensure uniqueness across different content hashes
    # This allows the same URL/content to be inserted with different descriptions
    base_id = doc.id or md5(cleaned_content.encode()).hexdigest()
          ...
          try:
            ...
            for idx, doc in enumerate(batch_docs):
              try:
                ...
                if doc.embedding is not None and isinstance(doc.embedding, list) and len(doc.embedding) == 0:
    ...
    try:
      with self.Session() as sess:
        ...
        stmt = (
          ...
          .values(
            meta_data=func.coalesce(self.table.c.meta_data, text("'{}'::jsonb")).op("||")(bindparam("md", type_=postgresql.JSONB)),
  ...
  def search(self, query: str, limit: int = 5, filters: Optional[Union[Dict[str, Any], List[FilterExpr]]] = None) -> List[Document]:
    """
    ...
    if self.search_type == SearchType.vector:
      ...
      log_error(f"Invalid search type '{self.search_type}'.")  # type: ignore[unreachable]
      return []

  async def async_search(self, query: str, limit: int = 5, filters: Optional[Union[Dict[str, Any], List[FilterExpr]]] = None) -> List[Document]:
  ...
  def vector_search(self, query: str, limit: int = 5, filters: Optional[Union[Dict[str, Any], List[FilterExpr]]] = None) -> List[Document]:
    """
    ...
    try:
      ...
      if query_embedding is None:
        log_error(f"Error getting embedding for Query: {query}")  # type: ignore[unreachable]
        return []
      ...
      if filters is not None:
        ...
        # Handle FilterExpr DSL
          ...
          sqlalchemy_conditions = [self._dsl_to_sqlalchemy(f.to_dict() if hasattr(f, "to_dict") else f, self.table) for f in filters]
      ...
      if self.distance == Distance.l2:
        ...
        log_error(f"Unknown distance metric: {self.distance}")  # type: ignore[unreachable]
        return []
  ...
  def keyword_search(self, query: str, limit: int = 5, filters: Optional[Union[Dict[str, Any], List[FilterExpr]]] = None) -> List[Document]:
    """
  ...
  def _create_vector_index(self, force_recreate: bool = False) -> None:
    ...
    if self.vector_index is None:
      log_debug("No vector index specified, skipping vector index optimization.")  # type: ignore[unreachable]
      return
    ...
    try:
      with self.Session() as sess, sess.begin():
        ...
        if isinstance(self.vector_index, Ivfflat):
          ...
          log_error(f"Unknown index type: {type(self.vector_index)}")  # type: ignore[unreachable]
          return
    except Exception as e:
  ...
  def _create_ivfflat_index(self, sess: Session, table_fullname: str, index_distance: str) -> None:
    ...
    create_index_sql = text(
      f'CREATE INDEX "{self.vector_index.name}" ON {table_fullname} USING ivfflat (embedding {index_distance}) WITH (lists = :num_lists);'
    )
  ...
  def delete_by_id(self, doc_id: str) -> bool:
    ...
    try:
      with self.Session() as sess, sess.begin():
        stmt = self.table.delete().where(self.table.c.id == doc_id)
        sess.execute(stmt)
        sess.commit()
        log_info(f"Deleted records with id '{doc_id}' from table '{self.table.fullname}'.")
        return True
    except Exception as e:
      log_error(f"Error deleting rows from table '{self.table.fullname}': {e}")
      sess.rollback()
