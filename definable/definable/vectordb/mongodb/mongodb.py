...
from definable.utils.log import log_debug, log_info, log_warning, logger
...
DRIVER_METADATA = DriverInfo(name="Definable", version=metadata.version("definable"))
...
class MongoDb(VectorDB):
  ...
  def __init__(
    ...
    **kwargs,
    ...
    Args:
        ...
        hybrid_rank_constant (int): Default rank constant (k) for Reciprocal Rank Fusion in hybrid search.
            This constant is added to the rank before taking the reciprocal. A common value is 60.
  ...
  def id_exists(self, doc_id: str) -> bool:
    ...
    try:
      ...
      log_debug(f"Document with ID '{doc_id}' {'exists' if exists else 'does not exist'}")
      return exists
    except Exception as e:
  ...
  def search(
    ...
    query_embedding = self.embedder.get_embedding(query)  # type: ignore[union-attr]
  ...
  def hybrid_search(
    ...
    try:
      ...
      logger.error(f"Error during hybrid search, potentially due to missing or misconfigured Atlas Search index for text search: {e}")
  ...
  async def async_search(self, query: str, limit: int = 5, filters: Optional[Union[Dict[str, Any], List[FilterExpr]]] = None) -> List[Document]:
    ...
    if isinstance(filters, List):
      log_warning("Filters Expressions are not supported in MongoDB. No filters will be applied.")
      filters = None
    query_embedding = await self.embedder.async_get_embedding(query)  # type: ignore[union-attr]
    if query_embedding is None:
      logger.error(f"Failed to generate embedding for query: {query}")
      return []
  ...
  def delete_by_id(self, doc_id: str) -> bool:
    ...
    try:
      ...
      if result.deleted_count > 0:
        log_info(f"Deleted {result.deleted_count} document(s) with ID '{doc_id}' from collection '{self.collection_name}'.")
        return True
      else:
        log_info(f"No documents found with ID '{doc_id}' to delete.")
        return True
    except Exception as e:
