...
class Qdrant(VectorDB):
  ...
  def create(self) -> None:
    ...
    if not self.exists():
      ...
      if self.search_type == SearchType.vector:
        ...
        vectors_config = {self.dense_vector_name: models.VectorParams(size=self.dimensions or 1536, distance=_distance)}  # type: ignore
      ...
      for key, value in filters.items():
        ...
        if isinstance(value, dict):
          ...
          for sub_key, sub_value in value.items():
            filter_conditions.append(models.FieldCondition(key=f"{key}.{sub_key}", match=models.MatchValue(value=sub_value)))
        else:
  ...
  def point_exists(self, doc_id: str) -> bool:
    ...
    try:
      log_info(f"Checking if point with ID '{doc_id}' (type: {type(doc_id)}) exists in collection '{self.collection}'")
      points = self.client.retrieve(collection_name=self.collection, ids=[doc_id], with_payload=False, with_vectors=False)
      log_info(f"Retrieved {len(points)} points for ID '{doc_id}'")
      if len(points) > 0:
        log_info(f"Found point with ID: {points[0].id} (type: {type(points[0].id)})")
      return len(points) > 0
    except Exception as e:
      log_info(f"Error checking if point {doc_id} exists: {e}")
      return False
  ...
  def delete_by_id(self, doc_id: str) -> bool:
    try:
      ...
      if not self.point_exists(doc_id):
        log_warning(f"Point with ID {doc_id} does not exist")
        return True
      ...
      log_info(f"Error deleting point with ID {doc_id}: {e}")
      return False
  ...
  def delete_by_name(self, name: str) -> bool:
    ...
    try:
      ...
      filter_condition = models.Filter(must=[models.FieldCondition(key="name", match=models.MatchValue(value=name))])
  ...
  def delete_by_metadata(self, metadata: Dict[str, Any]) -> bool:
    ...
    try:
      ...
      for key, value in metadata.items():
        ...
        filter_conditions.append(models.FieldCondition(key=f"meta_data.{key}", match=models.MatchValue(value=value)))
  ...
  def delete_by_content_id(self, content_id: str) -> bool:
    ...
    try:
      ...
      filter_condition = models.Filter(must=[models.FieldCondition(key="content_id", match=models.MatchValue(value=content_id))])
  ...
  def content_hash_exists(self, content_hash: str) -> bool:
    ...
    try:
      ...
      filter_condition = models.Filter(must=[models.FieldCondition(key="content_hash", match=models.MatchValue(value=content_hash))])
  ...
  def update_metadata(self, content_id: str, metadata: Dict[str, Any]) -> None:
    ...
    try:
      ...
      for operation in update_operations:
        self.client.set_payload(collection_name=self.collection, payload=operation.payload, points=operation.points)  # type: ignore[arg-type]
