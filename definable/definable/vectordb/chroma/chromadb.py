...
from definable.utils.log import log_debug, log_error, log_info, log_warning, logger
...
class ChromaDb(VectorDB):
  ...
  Args:
      ...
      doc_id: Unique identifier for this vector database instance.
  ...
  def search(self, query: str, limit: int = 5, filters: Optional[Union[Dict[str, Any], List[FilterExpr]]] = None) -> List[Document]:
    ...
    if self.search_type == SearchType.vector:
      ...
      logger.error(f"Invalid search type '{self.search_type}'.")  # type: ignore[unreachable]
      return []
  ...
  def _vector_search(self, query: str, limit: int = 5, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
    ...
    if query_embedding is None:
      logger.error(f"Error getting embedding for Query: {query}")  # type: ignore[unreachable]
      return []
  ...
  def delete_by_id(self, doc_id: str) -> bool:
    ...
    try:
      ...
      if not self.id_exists(doc_id):
        log_info(f"Document with ID '{doc_id}' not found")
        return False

      # Delete the document
      collection.delete(ids=[doc_id])
      log_info(f"Deleted document with ID '{doc_id}'")
      return True
    except Exception as e:
      logger.error(f"Error deleting document by ID '{doc_id}': {e}")
      return False
  ...
  def id_exists(self, doc_id: str) -> bool:
    ...
    try:
      ...
      logger.error(f"Error checking if ID '{doc_id}' exists: {e}")
      return False
  ...
  def update_metadata(self, content_id: str, metadata: Dict[str, Any]) -> None:
    ...
    try:
      ...
      try:
        ...
        if "object of type 'int' has no len()" in str(te):
          log_warning(f"ChromaDB internal error (version 0.5.0 bug): {te}. Cannot update metadata for content_id '{content_id}'.")
          return
        else:
          raise te
