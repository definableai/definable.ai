...
class Function(BaseModel):
  ...
  # If True, this tool runs sequentially even when parallel dispatch is active.
  # Use for tools with side effects that depend on execution order.
  ...
  def from_callable(cls, c: Callable, name: Optional[str] = None, strict: bool = False) -> "Function":
    ...
    from definable.utils.json_schema import get_json_schema
  ...
  def process_entrypoint(self, strict: bool = False):
    ...
    try:
      ...
      if strict:
        ...
        parameters["required"] = [
          name for name, param in sig.parameters.items() if param.default == param.empty and name != "self" and name not in excluded_params
        ]

      if params_set_by_user:
        self.parameters["additionalProperties"] = False
        if strict:
          self.parameters["required"] = [name for name in self.parameters["properties"] if name not in excluded_params]
        else:
          # Mark a field as required if it has no default value
  ...
  def _wrap_callable(func: Callable) -> Callable:
    ...
    if iscoroutinefunction(func) and pydantic_version < Version("2.10.0"):
      log_debug(
        f"Skipping validate_call for {func.__name__} because pydantic version is less than 2.10.0, "
        f"please consider upgrading to pydantic 2.10.0 or higher"
      )
      return func
...
class FunctionCall(BaseModel):
  ...
  def execute(self) -> FunctionExecutionResult:
    ...
    try:
      ...
      if isgenerator(result):
        ...
        execution_result = FunctionExecutionResult(status="success", result=self.result, updated_session_state=None)
      else:
        self.result = result
        ...
        if entrypoint_args.get("run_context") is not None:
          ...
          updated_session_state = run_context.session_state if run_context is not None and run_context.session_state is not None else None
        else:
          if self.function._session_state is not None:
            updated_session_state = self.function._session_state

        execution_result = FunctionExecutionResult(status="success", result=self.result, updated_session_state=updated_session_state)
  ...
  async def aexecute(self) -> FunctionExecutionResult:
    ...
    # Check cache if enabled and not a generator function
    if self.function.cache_results and not (isasyncgenfunction(self.function.entrypoint) or isgeneratorfunction(self.function.entrypoint)):
      cache_key = self.function._get_cache_key(entrypoint_args, self.arguments)
      cache_file = self.function._get_cache_file_path(cache_key)
      cached_result = self.function._get_cached_result(cache_file)
      if cached_result is not None:
