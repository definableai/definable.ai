  ...
  for message in messages:
    ...
    if message.role == "user":
      ...
      if message.images is not None:
        content: List[Any] = [TextChunk(type="text", text=str(message.content or ""))]
      ...
      if message.reasoning_content is not None:
        ...
        mistral_message = AssistantMessage(role="assistant", content=message.content, tool_calls=cast(Any, message.tool_calls))
      else:
        mistral_message = AssistantMessage(role=cast(Literal["assistant"], message.role), content=message.content)
    elif message.role == "system":
      mistral_message = SystemMessage(role="system", content=str(message.content or ""))
