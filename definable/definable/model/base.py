...
from definable.model.response import ModelResponse, ModelResponseEvent, ToolExecution
...
from definable.tool.function import Function, FunctionCall, FunctionExecutionResult, UserInputField
...
def _handle_agent_exception(a_exc: AgentRunException, additional_input: Optional[List[Message]] = None) -> None:
  ...
  if a_exc.user_message is not None:
    msg = Message(role="user", content=a_exc.user_message) if isinstance(a_exc.user_message, str) else a_exc.user_message
    additional_input.append(msg)

  if a_exc.agent_message is not None:
    msg = Message(role="assistant", content=a_exc.agent_message) if isinstance(a_exc.agent_message, str) else a_exc.agent_message
    additional_input.append(msg)
...
class Model(ABC):
    ...
    for attempt in range(self.retries + 1):
      try:
        ...
        if attempt < self.retries:
          ...
          log_warning(f"Model provider error (attempt {attempt + 1}/{self.retries + 1}): {e}. Retrying in {delay}s...")
          sleep(delay)
        else:
          if self.retries > 0:
  ...
  def _invoke_stream_with_retry(self, **kwargs) -> Iterator[ModelResponse]:
    ...
    for attempt in range(self.retries + 1):
      try:
        ...
        if attempt < self.retries:
          delay = self._get_retry_delay(attempt)
          log_warning(f"Model provider error during stream (attempt {attempt + 1}/{self.retries + 1}): {e}. Retrying in {delay}s...")
          sleep(delay)
        else:
          if self.retries > 0:
            log_error(f"Model provider error after {self.retries + 1} attempts: {e}")
  ...
  def response(
    ...
    try:
      ...
      while True:
        ...
        if _compression_manager is not None and _compression_manager.should_compress(messages, tools, model=self, response_format=response_format):
          ...
          for function_call_response in self.run_function_calls(
            ...
            if isinstance(function_call_response, ModelResponse):
              ...
              if (
                ...
                if function_call_response.event == ModelResponseEvent.tool_call_paused.value and run_response is not None:
  ...
  def _process_model_response(
    ...
    if provider_response.response_usage is not None:
      ...
      if model_response.response_usage.cost is None:
        ...
        model_response.response_usage.cost = calculate_cost(self.provider, self.id, model_response.response_usage)  # type: ignore[arg-type]
  ...
  def response_stream(
    ...
    send_media_to_model: bool = True,
    compression_manager: Optional["CompressionManager"] = None,
  ) -> Iterator[Union[ModelResponse, RunOutputEvent]]:
    """
    ...
    try:
      ...
      while True:
        ...
        if _compression_manager is not None and _compression_manager.should_compress(messages, tools, model=self, response_format=response_format):
          ...
          yield ModelResponse(event=ModelResponseEvent.compression_started.value)  # type: ignore[attr-defined]
          _compression_manager.compress(messages)
          # Emit compression completed event with stats
          yield ModelResponse(  # type: ignore[call-arg]
            event=ModelResponseEvent.compression_completed.value,  # type: ignore[attr-defined]
        ...
        yield ModelResponse(event=ModelResponseEvent.model_request_started.value)  # type: ignore[attr-defined]
        ...
        yield ModelResponse(  # type: ignore[call-arg]
          event=ModelResponseEvent.model_request_completed.value,  # type: ignore[attr-defined]
  ...
  def _populate_assistant_message_from_stream_data(self, assistant_message: Message, stream_data: MessageData) -> None:
    ...
    if stream_data.response_metrics is not None:
      ...
      if assistant_message.metrics.cost is None:
        ...
        assistant_message.metrics.cost = calculate_cost(self.provider, self.id, assistant_message.metrics)  # type: ignore[arg-type]
  ...
  def _populate_stream_data(self, stream_data: MessageData, model_response_delta: ModelResponse) -> Iterator[ModelResponse]:
    ...
    if model_response_delta.tool_calls is not None:
      if stream_data.response_tool_calls is None:
        stream_data.response_tool_calls = []  # type: ignore[unreachable]
    ...
    if model_response_delta.audio is not None and isinstance(model_response_delta.audio, Audio):
      if stream_data.response_audio is None:
        stream_data.response_audio = Audio(id=str(uuid4()), content=b"", transcript="")

      audio_response = model_response_delta.audio
    ...
    if assistant_message.tool_calls is not None:
      for tool_call in assistant_message.tool_calls:
        ...
        if _function_call.error is not None:
          messages.append(Message(role=self.tool_message_role, tool_call_id=_tool_call_id, content=_function_call.error))
          continue
        ...
        for item in function_execution_result.result:
          ...
          if isinstance(item, tuple(get_args(RunOutputEvent))):
            ...
            if isinstance(item, CustomEvent):
              ...
              item.tool_call_id = function_call.call_id  # type: ignore[attr-defined]
  ...
  def run_function_calls(
    ...
    for fc in function_calls:
      ...
      yield from self.run_function_call(function_call=fc, function_call_results=function_call_results, additional_input=additional_input)
  ...
  async def arun_function_call(
    ...
    try:
      ...
      elif function_call.function.tool_hooks is not None and any(iscoroutinefunction(f) for f in function_call.function.tool_hooks):
        result = await function_call.aexecute()
        success = result.status == "success"
      else:
  ...
  async def arun_function_calls(
    ...
    for fc in function_calls_to_run:
      ...
      if fc.function.name == "get_user_input" and fc.arguments and fc.arguments.get("user_input_fields") and not skip_pause_check:
    ...
    if skip_pause_check:
      ...
      function_calls_to_run = [
        fc
        for fc in function_calls_to_run
        if not (fc.function.requires_confirmation or fc.function.external_execution or fc.function.requires_user_input)
      ]

    results = await asyncio.gather(*(self.arun_function_call(fc) for fc in function_calls_to_run), return_exceptions=True)
              ...
              if j == i:  # This is our async generator
                if async_gen_index in async_generator_outputs:
                  ...
                  if error:
                    ...
                    log_error(f"Error while iterating async generator for {function_call.function.name}: {error}")
  ...
  def _prepare_function_calls(
    ...
    if model_response.tool_calls is None:
      model_response.tool_calls = []  # type: ignore[unreachable]
