...
class OpenAIChat(Model):
  ...
  # E.g. {"voice": "alloy", "format": "wav"}.
  # `format` must be one of `wav`, `mp3`, `flac`, `opus`, or `pcm16`.
  # `voice` must be one of `ash`, `ballad`, `coral`, `sage`, `verse`, `alloy`, `echo`, and `shimmer`.
  ...
  client: Optional[OpenAIClient] = None  # type: ignore[assignment]
  ...
  def _prepare_structured_output(
  ...
  ) -> tuple[List[Message], Optional[Union[Dict, Type[BaseModel]]]]:
    """Intercept structured output for models without native support.

    When a Pydantic BaseModel is passed as response_format but the model
    doesn't support native structured outputs, inject the JSON schema
    into the system prompt and set response_format to None.

    Models with native support (OpenAI) are unaffected.
    """
    ...
    injection = (
      ...
      f"{schema_str}\n"
      "Do not include any text, markdown formatting, or code fences. Output raw JSON only."
    )

    new_messages = list(messages)
    system_idx = next((i for i, m in enumerate(new_messages) if m.role == "system"), None)

    if system_idx is not None:
      original = new_messages[system_idx]
      new_messages[system_idx] = Message(role="system", content=(original.content or "") + injection)  # type: ignore[operator]
    else:
      new_messages.insert(0, Message(role="system", content=injection.strip()))

    # Use json_object mode to enforce valid JSON output
    return new_messages, {"type": "json_object"}
  ...
  def _format_message(self, message: Message, compress_tool_results: bool = False) -> Dict[str, Any]:
    ...
    if (message.images is not None and len(message.images) > 0) or (message.audio is not None and len(message.audio) > 0):
      # Ignore non-string message content
      # because we assume that the images/audio are already added to the message
  ...
  def invoke(
    ...
    """
    try:
      messages, response_format = self._prepare_structured_output(messages, response_format)
      ...
      provider_response = self.get_client().chat.completions.create(
        ...
        **self.get_request_params(response_format=response_format, tools=tools, tool_choice=tool_choice, run_response=run_response),
      )
    ...
    if hasattr(response_message, "audio") and response_message.audio is not None:  # type: ignore[unreachable]
      ...
      try:
        if isinstance(response_message.audio, dict):  # type: ignore[unreachable]
          model_response.audio = Audio(  # type: ignore[unreachable]
          ...
          model_response.audio = Audio(
            ...
            content=response_message.audio.data,  # type: ignore[arg-type]
