...
from definable.model.base import Model, RetryableModelProviderError
from definable.model.google.utils import MALFORMED_FUNCTION_CALL_GUIDANCE, GeminiFinishReason
...
from definable.tokens import count_schema_tokens, count_text_tokens, count_tool_tokens
...
class Gemini(Model):
  ...
  def get_request_params(
    ...
    if self.vertexai_search:
      ...
      builtin_tools.append(Tool(retrieval=Retrieval(vertex_ai_search=VertexAISearch(datastore=self.vertexai_search_datastore))))
  ...
  def count_tokens(
    ...
    if self.vertexai:
      ...
      if tools:
        total += count_tool_tokens(list(tools), self.id)
        ...
        for idx, tool_call in enumerate(message.tool_calls):
          if isinstance(content, list) and idx < len(content):
            ...
            if compress_tool_results:
              ...
              tc_content = compressed_from_tool_call or original_from_list
            else:
          ...
          message_parts.append(Part.from_function_response(name=tool_call["tool_name"], response={"result": tc_content}))
  ...
  def _format_file_for_message(self, file: File) -> Optional[Part]:
    if file.content and isinstance(file.content, bytes):
      ...
      if file_path.exists() and file_path.is_file():
        if file_path.stat().st_size < 20 * 1024 * 1024:  # 20MB
    ...
    **kwargs,
    ...
    if len(function_call_results) > 0:
      for idx, result in enumerate(function_call_results):
        ...
        combined_function_result.append({"tool_call_id": result.tool_call_id, "tool_name": result.tool_name, "content": compressed_content})
        if result.tool_name:
  ...
  def _parse_provider_response(self, response: GenerateContentResponse, **kwargs) -> ModelResponse:
    ...
    if response_message.parts is not None and len(response_message.parts) > 0:
      for part in response_message.parts:
        ...
        if hasattr(part, "inline_data") and part.inline_data is not None:
          if part.inline_data.mime_type and part.inline_data.mime_type.startswith("audio/"):
            ...
            model_response.images.append(Image(id=str(uuid4()), content=part.inline_data.data, mime_type=part.inline_data.mime_type))

        if hasattr(part, "function_call") and part.function_call is not None:
          call_id = part.function_call.id or str(uuid4())
          tool_call = {
            "id": call_id,
            "type": "function",
            "function": {
              "name": part.function_call.name,
              "arguments": json.dumps(part.function_call.args) if part.function_call.args is not None else "",
            },
          }
      ...
      if response.candidates and hasattr(response.candidates[0], "url_context_metadata") and response.candidates[0].url_context_metadata is not None:
  ...
  def _parse_provider_response_delta(self, response: GenerateContentResponse, **kwargs: Any) -> ModelResponse:
    ...
    if response_delta.candidates and len(response_delta.candidates) > 0:
      ...
      if hasattr(response_delta.candidates[0], "grounding_metadata") and response_delta.candidates[0].grounding_metadata is not None:
      ...
      if hasattr(response_delta.candidates[0], "url_context_metadata") and response_delta.candidates[0].url_context_metadata is not None:
  ...
  def _get_metrics(self, response_usage: GenerateContentResponseUsageMetadata) -> Metrics:
    """Parse the given Google Gemini usage into a Metrics object."""
    metrics = Metrics()
  ...
  def list_documents(self, store_name: str, page_size: int = 20) -> List[Any]:
    ...
    try:
      documents = []
      for doc in self.get_client().file_search_stores.documents.list(parent=store_name, config={"page_size": page_size}):
  ...
  async def async_list_documents(self, store_name: str, page_size: int = 20) -> List[Any]:
    ...
    try:
      documents = []
      async for doc in await self.get_client().aio.file_search_stores.documents.list(parent=store_name, config={"page_size": page_size}):
        documents.append(doc)
      log_debug(f"Found {len(documents)} documents in store {store_name}")
      return documents
    except Exception as e:
