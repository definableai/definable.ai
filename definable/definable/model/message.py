  ...
  def log(self, metrics: bool = True, level: Optional[str] = None, use_compressed_content: bool = False):
    ...
    if self.content:
      if use_compressed_content and self.compressed_content:
        ...
        else:  # isinstance(self.content, dict)
          _logger(json.dumps(self.content, indent=2))  # type: ignore[unreachable]
    if self.tool_calls:
      ...
      for tool_call in self.tool_calls:
        ...
        if tool_call_arguments:
          try:
            tool_call_args: dict = tool_call_arguments if isinstance(tool_call_arguments, dict) else json.loads(tool_call_arguments)
            if tool_call_args:
              # tool_call_args is guaranteed to be a dict here
    ...
    if metrics and self.metrics is not None and self.metrics != Metrics():
      ...
      if self.metrics.output_tokens and self.metrics.duration and self.metrics.duration > 0:
        _logger(f"* Tokens per second:           {self.metrics.output_tokens / self.metrics.duration:.4f} tokens/s")
