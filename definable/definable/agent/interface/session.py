...
class InterfaceSession:
  ...
  def truncate_history(self, max_messages: int) -> None:
    ...
    Tool-call-aware: never splits an assistant message with tool_calls
    from its corresponding tool result messages. If the naive cut point
    would orphan tool messages, the cut moves earlier to keep the
    complete tool-call group together.
    """
    if not self.messages or len(self.messages) <= max_messages:
      return

    cut = len(self.messages) - max_messages

    # Walk the cut point forward (earlier in list = fewer kept) until
    # we are NOT slicing between an assistant(tool_calls) and its tool results.
    # A tool message at position `cut` is orphaned if the preceding assistant
    # message (with tool_calls) would be discarded.
    while cut < len(self.messages):
      msg = self.messages[cut]
      if msg.role == "tool":
        # This tool message's assistant is before the cut — move cut earlier
        # to include the whole group.
        cut -= 1
        if cut <= 0:
          # Can't truncate without breaking groups — keep everything
          return
      else:
        break
