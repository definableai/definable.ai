"""Task model and TaskList for autonomous team execution (mode=tasks)."""
...
class TaskList:
  ...
  def all_terminal(self) -> bool:
    """Return True when every task is in a terminal state."""
    if not self.tasks:
      return False
    return all(t.status in TERMINAL_STATUSES for t in self.tasks)

  def get_summary_string(self) -> str:
    """Render the task list as a formatted string for the system prompt."""
    if not self.tasks:
  ...
  def _is_blocked(self, task: Task) -> bool:
    ...
    for dep_id in task.dependencies:
      ...
      if dep is None:
        return True  # Unknown dep — fail-closed
  ...
  def _update_blocked_statuses(self) -> None:
    ...
    so that ``all_terminal()`` can detect completion.
    """
    for task in self.tasks:
