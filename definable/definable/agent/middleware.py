...
import json
import logging
from typing import TYPE_CHECKING, AsyncGenerator, Awaitable, Callable, List, Optional, Protocol, runtime_checkable

if TYPE_CHECKING:
  from definable.knowledge import Document, Knowledge
  from definable.model.message import Message
  from definable.agent.events import RunContext, RunOutput, RunOutputEvent
...
class KnowledgeMiddleware:
  ...
  Example:
    ...
    middleware = KnowledgeMiddleware(Knowledge(
  ...
  def __init__(self, knowledge: "Knowledge"):
    ...
    Args:
      knowledge: Knowledge instance with retrieval settings.
    """
  ...
  async def __call__(
    ...
    try:
      documents = await self.knowledge.asearch(
        query=query,
        top_k=self.knowledge.top_k,
        rerank=self.knowledge.rerank,
      )
    except Exception:
    ...
    if self.knowledge.min_score is not None:
      documents = [doc for doc in documents if doc.reranking_score is not None and doc.reranking_score >= self.knowledge.min_score]
    ...
    if context.metadata is None:
      context.metadata = {}
    context.metadata["_knowledge_position"] = self.knowledge.context_position
  ...
  def _extract_query(self, messages: List["Message"]) -> Optional[str]:
    ...
    """
    if self.knowledge.query_from == "last_user":
      # Find last user message
      for msg in reversed(messages):
        if msg.role == "user" and msg.content:
          content = msg.content if isinstance(msg.content, str) else str(msg.content)
          return content[: self.knowledge.max_query_length]
      return None

    elif self.knowledge.query_from == "full_conversation":
      ...
      if user_contents:
        ...
        return full_query[: self.knowledge.max_query_length]
      return None
  ...
  def _format_context(self, documents: List["Document"]) -> str:
    ...
    """
    if self.knowledge.context_format == "xml":
      return self._format_xml(documents)
    elif self.knowledge.context_format == "markdown":
      return self._format_markdown(documents)
    elif self.knowledge.context_format == "json":
      return self._format_json(documents)
    return self._format_xml(documents)  # type: ignore[unreachable]
...
class StreamingMiddleware(Protocol):
  """Middleware that wraps the streaming event generator.

  Unlike :class:`Middleware` (which wraps the entire run and returns
  a single ``RunOutput``), ``StreamingMiddleware`` wraps the async
  event stream produced by the agentic loop.

  Implement this protocol to intercept, transform, or augment events
  during ``arun_stream()``.
  ...
  The agent checks ``isinstance(mw, StreamingMiddleware)`` to decide
  which protocol to use in streaming mode.
  """
